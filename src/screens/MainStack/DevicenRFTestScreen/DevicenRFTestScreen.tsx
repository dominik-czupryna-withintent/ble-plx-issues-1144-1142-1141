...
export function DevicenRFTestScreen(_props: DevicenRFTestScreenProps) {
  ...
  const [testScanDevicesState, setTestScanDevicesState] =
    useState<TestStateType>('WAITING');
  const [testDeviceConnectedState, setTestDeviceConnectedState] =
    useState<TestStateType>('WAITING');
  const [
    testDiscoverServicesAndCharacteristicsFoundState,
  ...
  const [
    testReadTimeTriggerDescriptorForDeviceState,
    setTestReadTimeTriggerDescriptorForDeviceState,
  ] = useState<TestStateType>('WAITING');
  const [
    testWriteTimeTriggerDescriptorForDeviceState,
    setTestWriteTimeTriggerDescriptorForDeviceState,
  ] = useState<TestStateType>('WAITING');

  const [testServicesForDeviceState, setTestServicesForDeviceState] =
    useState<TestStateType>('WAITING');
  const [testServicesForDeviceStateValue, setTestServicesForDeviceStateValue] =
    useState('');

  const [
    testCharacteristicsForDeviceState,
    setTestCharacteristicsForDeviceState,
  ] = useState<TestStateType>('WAITING');
  const [
    testCharacteristicsForDeviceStateValue,
    setTestCharacteristicsForDeviceStateValue,
  ] = useState('');

  const [testDescriptorsForDeviceState, setTestDescriptorsForDeviceState] =
    useState<TestStateType>('WAITING');
  const [
    testDescriptorsForDeviceStateValue,
    setTestDescriptorsForDeviceStateValue,
  ] = useState('');

  const [testIsDeviceConnectedStateState, setTestIsDeviceConnectedStateState] =
    useState<TestStateType>('WAITING');
  const [testOnDeviceDisconnectState, setTestOnDeviceDisconnectState] =
    useState<TestStateType>('WAITING');
  const [testConnectedDevicesState, setTestConnectedDevicesState] =
    useState<TestStateType>('WAITING');
  const [testRequestMTUForDeviceState, setTestRequestMTUForDeviceState] =
    useState<TestStateType>('WAITING');
  const [testCancelTransactionState, setTestCancelTransactionState] =
    useState<TestStateType>('WAITING');
  const [testReadRSSIForDeviceState, setTestReadRSSIForDeviceState] =
    useState<TestStateType>('WAITING');
  const [testGetDevicesState, setTestGetDevicesState] =
    useState<TestStateType>('WAITING');
  const [testBTStateState, setTestBTStateState] =
    useState<TestStateType>('WAITING');
  const [
    testRequestConnectionPriorityForDeviceState,
    setTestRequestConnectionPriorityForDeviceState,
  ] = useState<TestStateType>('WAITING');
  const [
    testStartCancelDeviceConnectionState,
    setTestStartCancelDeviceConnectionState,
  ] = useState<TestStateType>('WAITING');

  const [
    testMonitorCurrentTimeCharacteristicForDevice,
    setTestMonitorCurrentTimeCharacteristicForDevice,
  ] = useState<TestStateType>('WAITING');
  const [testDeviceDisconnectState, setTestDeviceDisconnectState] =
    useState<TestStateType>('WAITING');

  const [testEnableState, setTestEnableState] =
    useState<TestStateType>('WAITING');
  const [testDisableState, setTestDisableState] =
    useState<TestStateType>('WAITING');
  ...
  const onDeviceFound = (device: Device) => {
    if (
      ...
      expectedDeviceName.toLocaleLowerCase()
    ) {
  ...
  const startTestInfo = (testName: string) =>
    console.info('starting: ', testName);

  const runTest = (
    functionToRun: () => Promise<unknown>,
    stateSetter: Dispatch<TestStateType>,
    testName: string,
  ) => {
      ...
      BLEService.setupMonitor(
        ...
        async characteristic => {
          if (
            ...
            base64.decode(characteristic.value) === monitorExpectedMessage
          ) {
      ...
      () =>
        BLEService.getServicesForDevice().then(services => {
          ...
          setTestServicesForDeviceStateValue(
            JSON.stringify(
              services.map(({isPrimary, deviceID, id, uuid}) => ({
                isPrimary,
                deviceID,
                id,
                uuid,
  ...
  const startTestGetCharacteristicsForDeviceState = () =>
    runTest(
      () =>
        BLEService.getDescriptorsForDevice(
          deviceTimeService,
          currentTimeCharacteristic,
        ).then(descriptors => {
          if (!descriptors) {
  ...
  const startTestGetDescriptorsForDeviceState = () =>
    runTest(
      () =>
        BLEService.getCharacteristicsForDevice(deviceTimeService).then(
          characteristics => {
            if (!characteristics) {
  ...
  const getConnectedDevices = () =>
    BLEService.getConnectedDevices([deviceTimeService]).then(
      connectedDevices => {
        if (!connectedDevices) {
          throw new Error('getConnectedDevices error');
        }
        const accurateDevice = connectedDevices.find(
  ...
  const startGetConnectedDevices = () =>
    runTest(
      getConnectedDevices,
      setTestConnectedDevicesState,
      'startGetConnectedDevices',
    );
  ...
  const startTestCancelTransaction = () =>
    runTest(
      ...
      setTestCancelTransactionState,
      'startTestCancelTransaction',
    );

  const disconnectDevice = () =>
    runTest(
      BLEService.disconnectDevice,
      setTestDeviceDisconnectState,
      'disconnectDevice',
    );
  ...
  const startGetState = () =>
    runTest(getState, setTestBTStateState, 'startGetState');
  ...
  const onDeviceDisconnected = () => {
    ...
    const onDeviceDisconnectedSubscription = BLEService.onDeviceDisconnected(
      (error, device) => {
        if (error) {
  ...
  return (
    <ScreenDefaultContainer>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <TestStateDisplay
          label="Looking for device"
          state={testScanDevicesState}
        />
        <TestStateDisplay
          label="Device connected"
          state={testDeviceConnectedState}
        />
        <TestStateDisplay
        ...
        />
        <TestStateDisplay
          label="Is device connected"
          state={testIsDeviceConnectedStateState}
        />
        <TestStateDisplay
          label="Connected devices"
          state={testConnectedDevicesState}
        />
        <TestStateDisplay
          label="Request MTU for device"
          state={testRequestMTUForDeviceState}
        />
        <TestStateDisplay
          label="Test cancel transaction"
          state={testCancelTransactionState}
        />
        <TestStateDisplay
          label="Read RSSI for device"
          state={testReadRSSIForDeviceState}
        />
        ...
        <TestStateDisplay
          ...
          state={testDeviceDisconnectState}
        />
        <TestStateDisplay
          label="On device disconnect state"
          state={testOnDeviceDisconnectState}
        />
        <TestStateDisplay
          label="Cancel device connection"
          state={testStartCancelDeviceConnectionState}
        />
